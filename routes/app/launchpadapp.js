...
const LaunchpadApp = require("../../models/app/LaunchpadAppModel");

const router = express.Router();

const UPLOAD_DIR = path.join(process.cwd(), "uploads", "launchpadapp");
...
router.post("/launchpadapp-post", upload.single("image"), async (req, res) => {
  try {
    ...
    const item = await LaunchpadApp.create({
      name,
      link,
      couponCode,
      exclusive,
...
// READ ALL
router.get("/launchpadapp-get", async (req, res) => {
  try {
    const items = await LaunchpadApp.find().sort({ createdAt: -1 });
...
// READ ONE
router.get("/launchpadapp-get/:id", async (req, res) => {
  try {
    const item = await LaunchpadApp.findById(req.params.id);
...
router.put("/launchpadapp-put/:id", upload.single("image"), async (req, res) => {
  try {
    const item = await LaunchpadApp.findById(req.params.id);
    if (!item) {
      if (req.file?.filename) safeUnlink(path.join(UPLOAD_DIR, req.file.filename));
...
// DELETE
router.delete("/launchpadapp-delete/:id", async (req, res) => {
  try {
    const item = await LaunchpadApp.findById(req.params.id);
    if (!item) return res.status(404).json({ success: false, message: "Not found" });
    ...
    await LaunchpadApp.deleteOne({ _id: item._id });
